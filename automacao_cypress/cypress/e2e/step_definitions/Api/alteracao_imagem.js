...
Given('que faço login com as credenciais pré-definidas', () => {
  const username = 'luizloppeEss'; // Nome de usuário para login
...
When('eu faço uma requisição PUT para "/catalog/api/v1/product/image/723964127/luhenImg/ffffff?product_id=13" com a nova imagem', () => {
  ...
  cy.fixture(filePath, 'base64').then((fileContent) => {
    cy.request({
      method: 'PUT',
      url: `https://www.advantageonlineshopping.com/catalog/api/v1/product/image/723964127/luhenImg/ffffff?product_id=13`, // URL do endpoint para trocar a imagem do produto
      headers: {
